...
Suppose we have the following scenario: Cooks, Cashiers, and Customers are each modeled as a thread.
...
Implement a (concurrent multi‐threaded) solution to solve the problem and test it thoroughly.
...
void check_state() {
	...
	if(burger_count < 0) {
		printf("[check_state]\t ERROR: Negative burger count!\n");
		exit(40);
	}

	/* Assure that not more burgers than rack spaces are available */
	if(burger_count > RACK_HOLDER_SIZE) {
		printf("[check_state]\t ERROR: Rack overfull!\n");
		exit(41);
	}

	printf("[check_state]\t State consistent.\n");
}
